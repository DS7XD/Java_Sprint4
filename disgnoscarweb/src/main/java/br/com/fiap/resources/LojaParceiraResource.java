...
public class LojaParceiraResource {
    ...
    @Path("/{cnpjLoja}")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response atualizarLoja(Loja_ParceiraVO loja, @PathParam("cnpjLoja") String cnpjLoja) throws SQLException {
        try {
            loja.setCnpjLoja(cnpjLoja);
    ...
    @DELETE
    @Path("/{cnpjLoja}")
    public Response deletarLoja(@PathParam("cnpjLoja") String cnpjLoja) throws SQLException {
        try {
            lojaParceiraBO.deletarLoja(cnpjLoja);
